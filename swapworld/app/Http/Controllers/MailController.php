...
class MailController extends Controller {
   ...
   public function postMail(Request $req,EmailRequest $p){
         ...
         // dd(count($user));
         // dd($userId);
         $image_name = Profile::where('email','=',$req->email)->first();
         $image_path = '/profilePicture/'.$image_name->profilePicture; //image path assign into a variable...
         // dd($image_path);
         if(count($user) == 1){
            $userId = $user->userId;
            ...
            $data = array('pswrd'=>$fullPart,'receiver_email'=>$req->email,'image'=>$image_path);

            Mail::send('mail', $data, function($message) use($data){
               $message->to($data['receiver_email'], 'User')->subject
                  ('Reset Password')->attach($data['image']);
